...
float Ki = 0.0; // Iゲイン
float Kd = 5.0; // Dゲイン
float Ks = 30.0; // Pゲインの2乗
float Kvel = 0.0; // 速度FBゲイン0.1
float Kpos = 20.0; // 位置FBゲイン0.5
float Kpfilt = 0.2; // Pのfilter係数
float Kdfilt = 0.9; // Dのfilter係数
float Kposfilt = 0.7; // posのfilter係数
float Kposdec = 0.9; // posの減衰係数
float Kpwma = 1.0; // モータAのPWMの係数、基準は1.0 右側
float Kpwmb = 1.0; // モータBのPWMの係数、基準は1.0　左側
float Kt = 0.7; // 電池の消耗を考慮する係数、最初は~0.7、最後は~1.0、pwmの係数

float target = -7.5; // 目標値　大きくすると前にいく
...
void controlLoop(){
  ...
  */
  ...
  // PID制御
  // 目標から現在の角度を引いて偏差を求める
  target_offset = target_offset + 0.04 * (target_cmd - target_offset); // ゆっくり変化させる
  ...
  */
  ...
  pos = prevPos * Kposfilt + (1 - Kposfilt) * pos;
  vel = (pos - prevPos) / dt;
  prevPos = pos;
  ...
  // 左右のモータの違いを考慮
  turn_offset = turn_offset + 0.008 * (turn_cmd - turn_offset); // ゆっくり変化させる
...
void bleLoop(){
  ...
  if (paramChar.written()){ // 属性が書き換えられたかどうかを確認する
    ...
    // 操作コマンド
    if (cmd == "f") target_cmd += 1.3; // 最大で約1/3になる
    if (cmd == "b") target_cmd -= 1.3;
    if (cmd == "r"){
      turn_cmd += 50.;
      target_cmd += 0.9; // 少し前進する
    }
    if (cmd == "l"){
      turn_cmd -= 50.;
